...
* En cas d’échec de login :
    ...
    * Appliquer une politique :
       ...
       * après 5 échecs consécutifs → ajouter un délai (sleep).
       * après 10 échecs → verrouiller le compte 15 minutes (`lock_until`).
...
|  B2 | Échec simple incrémente compteur | Compte existant                                    | Tenter login avec mot de passe incorrect 1 fois     | `failed_attempts` = 1, `last_failed_at` mis à jour                |                          |
