...
!!! note "A faire"
    === "Modèle relationnel"
        Faites la même chose avec les tables CATEGORIES et CATEGORIES_TODOS
...
??? note ":eyes: tester"
    ...
    - [ ] Dans le template, mettre le code ci dessous contenant du bootstrap
...
??? warning "Fixer les erreurs "

    🚀 Installer Bootstrap avec Laravel 12, Vite et Vue/Inertia
    ...
    **1. Nettoyer et réinstaller les dépendances**
    ...
    npm i -D vite@^7 laravel-vite-plugin@^2 @vitejs/plugin-vue@^6 vue@^3
    ...
    **3. Ajouter Bootstrap en SCSS**

    👉 fichier : resources/sass/app.scss
    ```
    @use "bootstrap/scss/bootstrap";    
    ```

    👉 fichier : resources/sass/_variables.scss (optionnel, pour surcharger des variables Bootstrap)
    ```
    // Exemple d’override
    $primary: #0d6efd;
    $enable-rounded: true;

    // Import avec variables personnalisées
    ...
    **4. Charger Bootstrap JS**

    👉 fichier : resources/js/app.js
    ```js
    import 'bootstrap'; // JS Bootstrap (dropdowns, modals…)
    ```

    **5. Blade Layout**

    👉 fichier : resources/views/layouts/template.blade.php
    ```html
    <head>
        ...
...
??? note "Surcharge Bootstrap 🎨"

    Il est difficile de surcharger les thèmes Bootstrap avec une utilisation CDN ou avec `app.js`. Mais c'est possible à faire avec `sass`.<br />
    L'idée est de surcharger le CSS des class par défaut de bootstrap, par exemple `primary` ou `bg-dark` que l'on retrouve dans les balises des templates `<nav class="navbar bg-dark">`
    ...
    ```
    👉 contenu de _variables.scss

    ```php
        // ---- Variables perso (AUCUN @use ici) ----
        ...
        // Navbar
        // Astuce : .navbar-dark.bg-dark prend la couleur $dark
        $dark: #14532d; // vert foncé pour bg-dark
        // et couleurs du mode "dark" de la navbar :
        $navbar-dark-color: #e9ecef;
    ```

    ⚠️ ne pas oublier de faire un `npm run build` pour précompiler le sass.
...
## 1.7 Sauvegarde du formulaire et enregistrement en base
...
▶️ Adaptez votre Controleur pour prendre en compte le champ *Important*.
    ...
    ```php
     public function saveTodo(Request $request){
        ...
        if($texte){
            ...
            $todo->save();
            #Puis on ré-affiche la liste
            return redirect()->route('todo.liste');
        } else{
            #Si la zone de texte est vide, on affiche un message d'erreur en alimentant la variable $message présente dans le template
            return redirect()->route('todo.liste')->with('message', "Veuillez saisir une note à ajouter");
        }
