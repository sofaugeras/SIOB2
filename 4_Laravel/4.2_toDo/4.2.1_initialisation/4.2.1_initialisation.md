...
L’option `--create=todo` dans la première méthode permet d’indiquer le nom du modèle de la table à créer.<br />
...
!!! note "A faire"
    === "Modèle relationnel"
        Faites la même chose avec les tables CATEGORIE et CATEGORISE
...
php artisan make:controller CategoriessController
```
...
```
Il ne nous reste plus qu’à compiler les assets.
![vite](./data/vite.png)
...
:bulb: Dans `template.blade.php`<br />
dans la balise Head
```php
...
:arrow_forward: Pour tester, mettez en commentaires la route vers le contrôleur et remplacer par une route vers le template. 

??? note ":eyes: Tester"
    - [ ] Mettre en commentaire la route vers le controller Todo
    - [ ] Créer une route vers la vue template `Route :: view ('/test', 'template');`
    - [ ] Dans le template, mettre mettre un titre avec une classe Bootstrap bien visible `<a class="text-center" href="#">Ma Todo List</a>`
...
Dans votre template, ajouter dans le `head` la ligne ` @vite(['resources/js/app.css', 'resources/js/app.js'])`

pour utiliser les icones bootstrap dans vos pages : ajouter dans le fichier `app.scss` la ligne `@import 'bootstrap-icons/font/bootstrap-icons.css';`
![app.scss](./data/appscss.png)
...
## User story 1 : save Todo

Dans le premier form de la vue home, vous avez un input pour saisir un Todo
```php
<!-- Action -->
<form action="/action/add" method="post" class="add">
    <div class="input-group">
