...
:boom: Le souci est que cette méthode va nous obliger à revoir le nom des champs des noms pour laisser à Laravel le soin de faire ses jointures. Et donc normaliser les noms des clés primaires, qui devront impérativement être de `id` et normaliser également le nom des tables ``nom_table=nom_Controller``

<br />
:anger: Je vous invite à debugger par vous même votre application. Il va falloir supprimer la migration `categories` pour recréer une nouvelle migration "propre".
