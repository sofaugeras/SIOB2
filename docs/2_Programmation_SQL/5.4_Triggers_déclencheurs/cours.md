...
#### 1.3.1 Contraintes et vérifications de données
...
#### 1.3.3 Historisation des actions
...
#### 1.3.4 Mise à jour d'informations qui dépendent d'autres données
...
| Événement  | `OLD` | `NEW` |
|------------|-------|-------|
| `INSERT`   | ✗ non disponible | ✔ disponible |
| `UPDATE`   | ✔ disponible | ✔ disponible |
| `DELETE`   | ✔ disponible | ✗ non disponible |

Lors d'une insertion, `OLD` n'existe pas, puisque la ligne n'existe pas avant l'événement déclencheur. Dans le cas d'une suppression, c'est `NEW` qui n'existe pas, puisque la ligne n'existera plus après l'événement déclencheur.

### 2.5 Suppression des triggers
...
```sql
DELIMITER |
CREATE TRIGGER after_delete_lignecommande AFTER DELETE
